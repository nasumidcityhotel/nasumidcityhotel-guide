...
with open('index_downloaded.html', 'r', encoding='utf-8', errors='ignore') as f:
    html_content = f.read()
...
translations = {
    ...
    # Header
    "Nasu Mid-City Hotel Nearby Day-Trip Hot Spring Guide": "♨️ 那須ミッドシティホテル 近隣日帰り温泉ガイド",
    "From the Station Hotel to the Famous Waters of Nasu & Shiobara": "駅前ホテルから、那須・塩原の名湯めぐりへ。",
    "Nasu Mid-City Hotel features in-room modular baths only, but we invite you to enjoy the variety of unique nearby day-trip hot springs instead. From new facilities just 5 minutes away by car, to 5-star premium springs, and historic hidden hot springs, we offer a diverse range of onsen experiences.": "那須ミッドシティホテルは客室ユニットバスのみですが、<br>その分周辺には多様な日帰り温泉が充実しております。<br>駅チカの施設から五ツ星源泉、風情ある秘湯まで、<br>お客様のスタイルに合ったお気に入りの温泉のご利用をおすすめします。",
    
    # 温泉エリア
    "A. Station Area (5-15 min drive)": "A. 駅周辺エリア(車で5〜15分)",
    "Top Priority Area": "最優先エリア",
    "B. Nasu Area (30-40 min drive)": "B. 那須エリア(車で30〜40分)",
    "C. Shiobara Onsen Area (35-50 min drive)": "C. 塩原エリア(車で35〜50分)",
    ...
    "Day: Adults 800 yen": "昼 大人: 800円",
    "Night: Adults 650 yen": "夜 大人: 650円",
    ...
    "5-Star Source (Highest rating, held by only 1% in Japan)": "希少な「五ツ星源泉」(全国にわずか1%)",
    ...
    # Address / Contact
    "TEL: 0287-36-6802 / 0120-27-1126 (Reservation)": "TEL: 0287-36-6802",
    ...
    "Ootaka-no-Yu (10 min drive)5-star source, silky skin water": "大鷹の湯 (車で10分)<br>五ツ星源泉・美容液のような湯",
    "Easy Access Nearby": "アクセス良好(ホテル近隣)",
    "Nasu-shiobara Ekimae (5 min)Ootaka-no-Yu (10 min)Saika-no-Yu (10-15 min)": "那須塩原駅前温泉(車5分)<br>大鷹の湯(車10分)<br>彩花の湯(車10〜15分)",
    "Late Night Bathing": "夜遅くまで営業",
    "Senbonmatsu (until 11 PM)Ootaka-no-Yu (until 9 PM/check)": "千本松温泉(23時まで)<br>大鷹の湯(夜21時まで/要確認)",
    "Best Value": "コスパ抜群",
    "Shika-no-Yu (500 yen)Kojika-no-Yu (500 yen)": "鹿の湯 (500円)<br>小鹿の湯 (500円)",
    "Hidden & Scenic": "秘湯・絶景",
    "Omaru OnsenKita OnsenGensen-kan": "大丸温泉旅館<br>北温泉旅館<br>源泉館",
    "For Families": "ファミリー向け",
    "Shiobara Akatsuki-no-Yu (with pool)": "塩原あかつきの湯 (プールあり)",
    "Movie Locations": "映画のロケ地",
    "Kita Onsen(Thermae Romae)": "北温泉 <br>(テルマエ・ロマエ)",
    ...
    "A. It varies by facility. Please contact each facility in advance, especially for private baths or night use.": "A. 施設により異なります。貸切風呂や夜のご利用などは直接施設への確認をお願いします。",
    ...
    "A. Metal jewelry may tarnish. Also, those with sensitive skin should rinse off thoroughly after bathing.": "A. 金属アクセサリーは変色するため外してください。肌が弱い方は入浴後にシャワーで流すことをおすすめします。",
    ...
    "We hope this guide helps you with your Nasu/Shiobara hot spring tour.Enjoy a wonderful hot spring experience!": "このガイドが那須・塩原の温泉めぐりのお役に立てば幸いです。<br>素晴らしい温泉体験をお楽しみください！",
}
...
for k, v in translations.items():
    normalized_dict[clean_text(k)] = v

for en_tag in soup.find_all(attrs={"lang": "en"}):
    ja_tag = en_tag.find_previous_sibling(attrs={"lang": "ja"})
    if ja_tag:
        en_text_raw = en_tag.get_text(separator='')
        en_text = clean_text(en_text_raw)
        
        ja_text = normalized_dict.get(en_text)
        
        if not ja_text:
            for k, v in normalized_dict.items():
                if len(k) > 5 and k in en_text:
                    ja_text = v
                    break
...
for tag in soup.find_all(string=re.compile(r'횥ߥå')):
    tag.replace_with('那須ミッドシティホテル 日帰り温泉ガイド')

html_out = str(soup)
html_out = html_out.replace('??', '♨️').replace('?', '💡')
