...
# 1. <title> の強制置換
html = re.sub(r'<title>.*?</title>', '<title>♨️ 那須ミッドシティホテル 近隣日帰り温泉ガイド</title>', html, flags=re.DOTALL)

# 2. lang="ja" の後ろに lang="en" が続くペアの処理
def replace_ja(match):
    ja_tag = match.group(1) # '<span lang="ja">' のようになる全体
    en_content = match.group(3).strip()
    
    # \n 等を正規化して検索
    mapped = translations.get(en_content, None)
    
    if not mapped:
        # 部分一致や近似マッチを探す（簡易的）
        for k, v in translations.items():
            if k in en_content or en_content in k:
                mapped = v
                break

    if not mapped:
        # どうしても見つからなければ化けを強引に消す（あるいは英語をそのまま）
        mapped = en_content
        
    return f'{ja_tag}{mapped}</{match.group(2)}>'

# パターン: (<タグ lang="ja"[^>]*>).*?(</タグ>)\s*(<タグ lang="en"[^>]*>(.*?)</タグ>)
# 注: beautifulsoupを使わない簡易パース

def do_replacement(html):
    # h1, h2, h3, h4, p, span, div, li, strong などのタグペアを検索
    for tag in ['h1', 'h2', 'h3', 'h4', 'span', 'div', 'p', 'li', 'strong']:
        # 例: <span lang="ja">化け</span>\s*<span lang="en">英語</span>
        pattern = re.compile(rf'(<{tag}[^>]*?lang="ja"[^>]*>)\s*.*?\s*(</{tag}>)(\s*<{tag}[^>]*?lang="en"[^>]*>\s*(.*?)\s*</{tag}>)', re.DOTALL)
        
        def replacer(m):
            ja_start = m.group(1)
            ja_end = m.group(2)
            en_full = m.group(3)
            en_content = m.group(4).strip()
            
            ja_content = translations.get(en_content, en_content) # 無ければ英語をそのまま
            
            return f"{ja_start}{ja_content}{ja_end}{en_full}"
            
        html = pattern.sub(replacer, html)
        
    return html

html = do_replacement(html)
...
print("Fixed HTML written to index_fixed.html")
